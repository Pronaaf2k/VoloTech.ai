...
class FlowCurve extends THREE.Curve<THREE.Vector3> {
  constructor() { super(); this.arcLengthDivisions = 640; }
...
function Sculpture({ onReady, reduced }: { onReady: () => void; reduced: boolean }) {
...
export default function FlowSculpture({ onReady, onFailure, reduced }: { onReady: () => void; onFailure: () => void; reduced: boolean }) {
  ...
  return <div className="sculpture-stage" ref={container} role="img" aria-label="Six orange metal ribbons forming a flowing sculpture that responds to your pointer">
    <Canvas camera={{ position: [0, 0, 10.8], fov: 42 }} dpr={1} frameloop="demand" gl={{ alpha: true, antialias: true, powerPreference: 'low-power' }} fallback={null} onCreated={({ gl }) => { gl.domElement.addEventListener("webglcontextlost", onFailure, { once: true }); }}><Suspense fallback={null}><StudioLighting /><Sculpture reduced={reduced} onReady={onReady} /><FrameScheduler active={visible && foreground && !reduced} /></Suspense></Canvas>
  </div>;
}
function FrameScheduler({ active }: { active: boolean }) {
